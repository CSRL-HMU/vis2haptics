...
DEFAULT_CENTRE_CROP_SIZE = 620 #200
DEFAULT_RESIZED_IMAGE_SIZE = 620# 100
...
# Fortwsh eikonwn apo to .csv arxeio
def load_image_targets_from_csv(csv_path: Path, header: bool = True) -> Dict[str, Any]:
    ...
    """
    image_targets = {}
    with csv_path.open('r') as f: # anoigei to .csv gia read
        lines = f.readlines() # diavazei oles tis grammes
        start_line = 0
        # If there is a header, skip the first line
        if header:
            header_line = lines[0].strip().split(',')
            # print(f'Header line of csv {csv_path} : {header_line}') # kanei print to header_line
            start_line = 1
        for line in lines[start_line:]:
            line = line.strip().split(',') # diaxwrizei th kathe grammh me komma
            image_path = line[0]
            # print(line[1:])
            image_targets[image_path] = np.array([float(x) for x in line[1:]], dtype=np.float32) #metatrepei kathe timh se dekadiko
    return image_targets


# maps image paths to their target values
class RegressionImageFolder(datasets.ImageFolder):
    ...
    def __init__(
        ...
        self.targets = [image_targets[str(path)] for path in paths] # ta target values apothhkeuontai sto image_targets
...
class RegressionTaskData:
    ...
    def visualise_image(self):
        ...
        """
        # Ektupwnei tis diastaseis ths eikonas kai tou target ths
